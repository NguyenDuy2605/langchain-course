...
def get_product_price(product: str) -> float:
    """Loop up the price of a product in the catalog.""" # đây là docstring
...
def apply_discount(price: float, discount_tier: str) -> float:
    ...
    Available tier: gold, silver, bronze."""
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        #print(f"Khi match: {action_input_match.group()}")
        ...
        if tool_name not in tools:
            observation = f"Error: Tool '{tool_name}' not found. Availabel tools: {list[str](tools.keys())}"
        else:
...
if __name__ == "__main__":
    print("Hello Langchain Agent(.bind_tool!)")
    print()
